...
import xin.zhuyao.wechat_bot_public.protocol.*;
import xin.zhuyao.wechat_bot_public.utils.WXHttpExecutor;
...
final class WeChatApi {
    ...
    File webwxgetmsgimg(long msgId, String type) {
        ...
        try {
            ...
            if (!XTools.strEmpty(suffix)) {
                File file = XTools.fileToFile(imgFile, folder.getAbsolutePath() + "/peng" + File.separator + String.format("image-%s-%s.%s", String.valueOf(type), msgId, suffix));
                imgFile.delete();
                return file;
            }
        } catch (IOException e) {
    ...
    File webwxgetvoice(long msgId) {
        ...
        request.query("skey", skey);
        request.query("pass_ticket", passticket);
        return XTools.http(httpExecutor, request).file(folder.getAbsolutePath() + "/mp3" + File.separator + String.format("voice-%s.mp3", msgId));
    }
    ...
    File webwxgetvideo(long msgId) {
        ...
        return XTools.http(httpExecutor, request).file(folder.getAbsolutePath() + "/mp4" + File.separator + String.format("video-%d.mp4", msgId));
    }
